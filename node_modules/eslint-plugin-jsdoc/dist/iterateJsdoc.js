...
const parseComment = (commentNode, indent, trim = true) => {
  ...
  return (0, _commentParser.default)(`${indent}/*${commentNode.value}${indent}*/`, {
    ...
    (str, data) => {
      ...
      const result = str.match(/^\s?((.|\s)+)?/u); // Always has at least whitespace due to `indent` we've added
...
const getUtils = (node, jsdoc, jsdocNode, {
  ...
  utils.tagMightHaveEitherTypeOrNamepath = tagName => {
    return _jsdocUtils.default.tagMightHaveEitherTypeOrNamepath(mode, tagName);
  };
  ...
  utils.tagMightHaveAType = tagName => {
    return _jsdocUtils.default.tagMightHaveAType(mode, tagName);
  };
